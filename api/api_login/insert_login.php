...
// make sure data is not empty
        ...
        !empty($data->amphure_id) &&
        !empty($data->districts_id)
     
    ){
        
        // set product property values
        $hash_password = password_hash($data->user_passwords,PASSWORD_DEFAULT);
        $insert_login->user_username = ltrim($data->user_username);
        ...
        $insert_login->amphure_id = $data->amphure_id;
        $insert_login->districts_id = $data->districts_id;
        $insert_login->roles_id = 1;
        if(strlen($data->user_passwords) >= 6){
        if($insert_login->insert_login()){
      
            http_response_code(200);
            echo json_encode(array("message" => "สมัครบัญชีผู้ใช้สำเร็จ"),JSON_UNESCAPED_UNICODE);
        }
        else{
            // http_response_code(401);
            echo json_encode(array("message" => "บัญชีผู้ใช้นี้เปิดใช้งานแล้ว - โปรดลองป้อนบัญชีใหม่อีกครั้ง"),JSON_UNESCAPED_UNICODE);
        }
        }
        else{
            // http_response_code(401);
            echo json_encode(array("message" => "กรอกรหัสผ่านไม่น้อยกว่า 6 ตัวอักษร"),JSON_UNESCAPED_UNICODE);
        }
    }
        ...
        echo json_encode(array("message" => "Unable to create product. Data is incomplete."),JSON_UNESCAPED_UNICODE);
    }
